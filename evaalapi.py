...
def trial_status(op, key):
    ...
    # ts[t] = {k: v for k, v in trials[t].items() if k not in ('log', 'dataf', 'response_data')}
    if op == 'complete':
        for t in trials:
            ts[t] = {}
...
def trial_nextdata(trialname):
    ...
    if p > 0:  # p == 0 for first state request call
        ## Force no-faster-than-real-time behaviour
        # if V > 1 and (c-p) < h:          # client is faster than real-time
    ...
    if state in ('running', 'finished'):
        ...
        if 'label' in request.args:
            label = escape(request.args['label'])
